...
def to_tanh_space(x, box):
    ...
    """
    # _box_mul = tuple(a*0.5 for a in tuple(map(op.sub, box[1], box[0]))) #(box[1] - box[0]) * 0.5
    # _box_plus = tuple(a*0.5 for a in tuple(map(op.add, box[1], box[0]))) #(box[1] + box[0]) * 0.5
    # return atanh((x - torch.from_numpy(np.asarray(_box_plus)[:, np.newaxis, np.newaxis]).cuda().float() ) / torch.from_numpy(np.asarray(_box_mul)[:, np.newaxis, np.newaxis]).cuda().float() )
...
def from_tanh_space(x, box):
    ...
    """
    # _box_mul = tuple(a*0.5 for a in tuple(map(op.sub, box[1], box[0]))) #(box[1] - box[0]) * 0.5
    # _box_plus = tuple(a*0.5 for a in tuple(map(op.add, box[0], box[1]))) #(box[1] + box[0]) * 0.5
    # return torch.tanh(x).float() * torch.from_numpy(np.asarray(_box_mul)[:, np.newaxis, np.newaxis]).cuda().float() + torch.from_numpy(np.asarray(_box_plus)[:, np.newaxis, np.newaxis]).cuda().float()
    _box_mul = (box[1] - box[0]) * 0.5
    _box_plus = (box[1] + box[0]) * 0.5
...
class L2Adversary(object):
    ...
    def __init__(self, num_classes, targeted=True, confidence=0.0, c_range=(1e-3, 1e10),
        ...
        self.box = box #tuple(map(float, box))  # type: Tuple[float, float]
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        pert_outputs_var, detector_output = model(advxs_var)  # type: Variable
        # the original inputs
        ...
        ## Eval the detector loss
        if detector_output.shape[0] > 1:
            detector_output = detector_output.squeeze()
        else:
            detector_output = detector_output[0]
        
        detector_loss = F.binary_cross_entropy_with_logits(detector_output, torch.zeros(size=(detector_output.shape[0],)).cuda())

        batch_loss_var = torch.sum(perts_norm_var + c_var * f_var) + 1000.0*detector_loss
